"""突破后中期整理 - 放量上涨后近15日窄幅整理，大流通市值

策略逻辑:
  1. 近10个交易日，日平均成交额 > 2亿
  2. 近10个交易日，每日成交额都 > 1.5亿
  3. 近20个交易日，至少有一天涨幅 > 6%
  4. 近15个交易日，每日涨幅在 -6% 到 2% 之间 (整理)
  5. 流通市值 > 100亿 (在 BASIC_FILTER 中检查)

注: 条件 3 的 >6% 上涨日必然落在近15日整理窗口之外 (即 16~20 日前)，
形成"先突破、后整理"的形态。
...
META = {
    "id": "lab_breakout_mid_consolidation",
    "name": "突破后中期整理",
    "description": "近20日有强势上涨 + 近15日放量窄幅整理 + 流通市值>100亿",
    ...
    "scoring": {"_avg_amount_10d": 0.4, "_max_change_20d": 0.3, "momentum_20d": 0.3},
    "order_by": "score",
    "descending": True,
    "limit": 50,
}
...
def filter_history(df: pl.DataFrame, params: dict) -> pl.DataFrame:
    ...
    consol_high = float(params.get("consolidation_high", 2.0)) / 100.0
    ...
    stats = (
        ...
        .agg(
            ...
            # 条件 3: 近20日最大涨幅 (至少一天达标 = 最大值达标)
            pl.col("change_pct").tail(20).max().alias("_max_change_20d"),
            # 条件 4: 近15日涨幅范围 (每日都在区间内 = min>=下限 且 max<=上限)
            pl.col("change_pct").tail(15).min().alias("_min_change_15d"),
        ...
        .filter(
            ...
            & (pl.col("_max_change_20d") > max_chg_min)
            & (pl.col("_min_change_15d") >= consol_low)
            & (pl.col("_max_change_15d") <= consol_high)
        )
    )
