"""强势整理 - 放量上涨后窄幅整理，大流通市值

策略逻辑:
  1. 近10个交易日，日平均成交额 > 2亿
  2. 近10个交易日，每日成交额都 > 1.5亿
  3. 近10个交易日，至少有一天涨幅 > 6%
  4. 近4个交易日，每日涨幅在 -2% 到 1% 之间 (整理)
...
META = {
    "id": "lab_strong_consolidation",
    "name": "强势整理",
    "description": "近10日放量强势 + 近4日窄幅整理 + 流通市值>100亿",
    "tags": ["强势", "整理", "放量", "大市值"],
    "asset_types": ["stock"],
    "timeframes": ["1d"],
    "params": [
    ...
    "scoring": {"_avg_amount_10d": 0.4, "_max_change_10d": 0.3, "momentum_20d": 0.3},
    "order_by": "score",
    "descending": True,
    "limit": 50,
}
...
def filter_history(df: pl.DataFrame, params: dict) -> pl.DataFrame:
    ...
    max_chg_min = float(params.get("max_change_min", 6.0)) / 100.0
    consol_low = float(params.get("consolidation_low", -2.0)) / 100.0
    consol_high = float(params.get("consolidation_high", 1.0)) / 100.0

    # 按 symbol 分组，取近 10 个交易日的统计量
    stats = (
        ...
        .agg(
            ...
            # 条件 3: 近10日最大涨幅 (至少一天达标 = 最大值达标)
            pl.col("change_pct").tail(10).max().alias("_max_change_10d"),
            # 条件 4: 近4日涨幅范围 (每日都在区间内 = min>=下限 且 max<=上限)
            pl.col("change_pct").tail(4).min().alias("_min_change_4d"),
            pl.col("change_pct").tail(4).max().alias("_max_change_4d"),
            # 数据完整性: 确保有足够交易日
        ...
        .filter(
            ...
            & (pl.col("_max_change_10d") > max_chg_min)
            & (pl.col("_min_change_4d") >= consol_low)
            & (pl.col("_max_change_4d") <= consol_high)
        )
    )
